(function() {
  var harViewer = Backbone.Marionette.View.extend({
    ...
    viewHar: function(event) {
      var x = window.open('', '', 'location=no,toolbar=no,scrollbars=yes,fullscreen=yes');
      x.document.write('<html><head>'+
        ...
        '<title>Analytics HAR analyzer</title>'+
        '</head>'+
        '<body>'+
        '  <div id="content"></div>'+
    ...
    template: function (data) {
      ...
      if (links.length) {
        var ret = '<h3 class="pane__section-title">Analytics requests</h3>' +
